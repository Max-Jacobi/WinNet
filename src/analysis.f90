...
module analysis
  ...
  integer                               :: cum_flowcnt                           !< cumulative flow printing counter
...
subroutine output_iteration(cnt,ctime,T9,rho_b,entropy,Rkm,Y,pf)
  ...
  ! Accumulate flows once per iteration
  ! (skip NSE evolution, where vals holds a stale Jacobian)
  if (((flow_every.gt.0) .or. (cum_flow_every.gt.0) &
#ifdef USE_HDF5
      .or. (h_flow_every.gt.0) .or. (h_cum_flow_every.gt.0) &
#endif
      ) .and. (cnt.ne.0) .and. (evolution_mode.ne.EM_NSE)) then
  ...
  if ((cum_flow_every.gt.0) .and. (cnt.ne.0)) then
     if(mod(cnt,cum_flow_every).eq.0) then
        call flowprint(ctime,T9,rho_b,cum_flowcnt,.true.)
        cum_flowcnt = cum_flowcnt + 1
     end if
  end if
